...
export class AuthService {
    ...
    login() {
        const lock = new Auth0Lock(
            ...
            (error) => this.log.error('[AuthService] Error while calling Auth0: ' + JSON.stringify(error))
        );
        ...
        lock.on('authenticated', authResult => {
            ...
            lock.getUserInfo(authResult.accessToken, function (error, profile) {
                if (error) {
                    me.log.error('[AuthService] Error while trying to get user info' + JSON.stringify(error));
                    me.router.navigate(['/']);
                    return;
                }
